...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        try {
            ...
            if(!args[0] || isNaN(args[0])) return interaction.reply({ content: `👎 **You forgot to add the seeking-time!** Usage: \`${prefix}seek <Time-In-S>\``}).catch(() => null);
            ...
            interaction.reply({content: `⏩ **Seeked to \`${client.formatDuration(newPos)}\`**!`}).catch(() => null);
        } catch(e) { 
            console.error(e);
