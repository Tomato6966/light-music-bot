...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        ...
        if(!args[0] || isNaN(args[0]) || Number(args[0]) > queue.tracks.length)
            return interaction.reply({ content: `👎 **There are just ${queue.tracks.length} Songs in the Queue, can't skip to ${args[0]}th Song.**`})
        ...
        return interaction.reply(`⏭️ **Successfully skipped ${args[0]} Track(s)**`).catch(() => null);
    },
};