...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        ...
        if(!args[0] || isNaN(args[0]) || Number(args[0]) < 1 || !args[1] || isNaN(args[1]) || Number(args[1]) < 1)
            return interaction.reply(`👎 **From where to where shall I move?** Usage: \`${prefix}move <fromPosNr.> <toPosNr.>\``).catch(() => null);
        
        queue.tracks = arrayMove(queue.tracks, args[0], args[1])
        
        return interaction.reply(`🪣 **Successfully moved the \`${client.queuePos(args[0])} Song\` to \`${client.queuePos(args[1])} Position\` in the Queue.**`).catch(() => null);
    },
};
