...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        try {
            ...
            if(!args[0] || isNaN(args[0])) return interaction.reply({ content: `👎 **You forgot to add the forwarding-time!** Usage: \`${prefix}forward <Time-In-S>\``}).catch(() => null);
            ...
            interaction.reply({content: `⏩ **Forwarded for \`${args[0]}s\` to \`${client.formatDuration(newPos)}\`**!`}).catch(() => null);
        } catch(e) { 
            console.error(e);
