...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        ...
        if(!args[0] || isNaN(args[0]) || Number(args[0]) > queue.tracks.length)
            return interaction.reply({ content: `👎 **There are just ${queue.tracks.length} Songs in the Queue, can't remove the ${!isNaN(args[0]) ? client.queuePos(Number(args[0])) : args[0]} Song.**`})
        
        queue.skipped = true;

        queue.tracks.splice(args[0], 1)
        
        return interaction.reply(`⏭️ **Successfully removed the ${client.queuePos(Number(args[0]))} Track!**`).catch(() => null);
    },
};