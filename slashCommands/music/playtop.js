...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        ...
        if(!queue.tracks || queue.tracks.length <= 1) { 
            return interaction.reply(`👎 **Not enough Songs in the Queue**`).catch(() => null);
        }
        const track = args.join(" ");
        if(!args[0]) return interaction.reply(`👎 Please add the wished Music via: \`${prefix}playtop <Name/Link>\``).catch(() => null);
        ...
        try {
            ...
            return interaction.reply(`❌ Could not play the Song because: \`\`\`${e.interaction || e}`.substring(0, 1950) + `\`\`\``).catch(() => null);
        }
    },
};